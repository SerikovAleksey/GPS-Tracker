...
uint16_t *FLASH_Read(uint32_t Address, uint16_t *buf, uint16_t len)
{
	...
	if ((uint32_t)Address == (uint32_t)ADDR_FLASH_PAGE_127)
	{
		add += (uint32_t)4;
		size = len - 2;
	}
...
uint16_t *buf_u8_to_u16(uint8_t *buf, uint16_t len_buf, uint16_t *buf_u16)
{
...
uint8_t *buf_u16_to_u8(uint16_t *buf, uint16_t len_buf, uint8_t *buf_u8)
{
...
{
	uint16_t amount_lists = *((uint32_t*)(ADDR_FLASH_PAGE_127)) - '0';
	uint16_t len1 = *((uint32_t*)(ADDR_FLASH_PAGE_127 + 2))- '0';
	uint16_t size = 512;
